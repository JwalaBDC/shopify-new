class TabSliderInterface extends HTMLElement {
    ...
    this.cardObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        if (target.hasAttribute("tab-slider-card")) {
          target.setAttribute("aria-hidden", entry.isIntersecting ? "false" : "true");
        }
      });
  ...
  createNextClickHandler() {
    return () => {
      const lastVisibleCardIndex = this.cards.findIndex(card => card.getAttribute("aria-hidden") === "false");
  ...
  createPrevClickHandler() {
    return () => {
      const firstVisibleCardIndex = this.cards.findIndex(card => card.getAttribute("aria-hidden") === "false");
  ...
  smoothScrollTo(card, goto = "") {
    if (goto === "first") {
      ...
      this.cardList.scrollTo({
        left: this.cardList.scrollWidth,
        behavior: "smooth"
      });
    } else if (card) {
      this.cardList.scrollTo({
