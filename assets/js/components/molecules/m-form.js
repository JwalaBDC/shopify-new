let inputElements = document.querySelectorAll("[type='search']:not(.-allowEmoji), [type='text']:not(.-allowEmoji)");
if (inputElements.length) {
  inputElements.forEach(inputElement => {
    function validateInput() {
      const inputValue = inputElement.value;
      const emojiRegex = /[\uD800-\uDBFF][\uDC00-\uDFFF]|\p{Emoji_Presentation}|\p{Emoji}\uFE0F/gu; // Emoji regex pattern
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (allowOnlyNumberText.length > 0) {
    allowOnlyNumberText.forEach(input => input.addEventListener("keypress", allowOnlyNumbersText));
    allowOnlyNumberText.forEach(input => input.addEventListener("input", allowOnlyNumbersText));
  }
  var numberInputs = document.querySelectorAll(".-allowOnlyNumber");
  if (numberInputs.length > 0) {
    numberInputs.forEach(input => input.addEventListener("keypress", allowOnlyNumbers));
    numberInputs.forEach(input => input.addEventListener("paste", handlePasteOnlyNumbers));
  }
  var textInputs = document.querySelectorAll(".-allowOnlyText");
  if (textInputs.length > 0) {
    textInputs.forEach(input => input.addEventListener("keypress", allowOnlyLetters));
    textInputs.forEach(input => input.addEventListener("input", allowOnlyLetters));
  }
});